...
fn test_chars_to_u64() {
    ...
    assert_eq!(
        Ok(500000000000000000),
        <Module<TestRuntime>>::chars_to_u64(correct_balance)
    );

    let correct_balance = vec!['a', '2'];
    assert_eq!(
        Err("Wrong u64 balance data format"),
        <Module<TestRuntime>>::chars_to_u64(correct_balance)
    );
}
...
fn test_parse_multi_balances() {
    ...
    assert_eq!(
        Some(vec![vec!['1', '2'], vec!['2', '1']]),
        <Module<TestRuntime>>::parse_multi_balances(double_balances)
    );
}
...
fn test_parse_balance() {
    ...
    assert_eq!(
        Some(vec!['1', '2']),
        <Module<TestRuntime>>::parse_balance(balance)
    );
}
